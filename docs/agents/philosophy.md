...
**Channel 2 — MCP (interactive gates).** When a workflow reaches a mandatory pause, the orchestrator calls an MCP tool. For operational gates (design, execution-plan, qa-escalation), it calls `request_approval` — a blocking call where the agent suspends and waits for a verdict (`approved`, `rejected`, or `needs_revision`). For document reviews (epic requirements, story definitions), versions are created via the CLI and reviews are monitored via polling. The dashboard displays the artifact, a human reviews it, and the verdict flows back to unblock the agent. These gates are the only blocking operations in the system. Everything else is fire-and-forget.
