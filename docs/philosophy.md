...
- **Human-in-the-Loop (HITL)** — Automation accelerates execution, but certain decisions require human judgment. At critical phase transitions, Oraculo pauses and presents artifacts to a human reviewer through the dashboard's approval gates. The agent calls `oraculo tools approval request`, the dashboard displays the artifact for review, and the agent enters `awaiting_approval`. Workflow does not advance until a verdict is delivered. Verdicts are: `approved` (advance to next phase), `rejected` (return to the generator phase), or `needs_revision` (return with reviewer comments). The five approval gates are: `epic-requirements`, `story-definition`, `design`, `execution-plan`, and `qa-escalation`.
